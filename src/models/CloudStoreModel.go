...
func (c *CloudStore) GetPublicBucket() (publicBucket string) {
	storeType := helper.ConfigCate(GlobalSys.StoreType)
	modelConfig := NewConfig()
	config := modelConfig.GetGlobalConfigWithStruct(storeType)
	c = &CloudStore{
	...
	switch c.StoreType {
	case StoreOss:
		cfg := c.config.(*ConfigOss)
		bucket = cfg.PublicBucket
	case StoreObs:
		cfg := c.config.(*ConfigObs)
		bucket = cfg.PublicBucket
	case StoreQiniu:
		cfg := c.config.(*ConfigQiniu)
		bucket = cfg.PublicBucket
	case StoreUpyun:
		cfg := c.config.(*ConfigUpYun)
		bucket = cfg.PublicBucket
	case StoreMinio:
		cfg := c.config.(*ConfigMinio)
		bucket = cfg.PublicBucket
	case StoreBos:
		cfg := c.config.(*ConfigBos)
		bucket = cfg.PublicBucket
	case StoreCos:
		cfg := c.config.(*ConfigCos)
		bucket = cfg.PublicBucket
	}
	return bucket
}